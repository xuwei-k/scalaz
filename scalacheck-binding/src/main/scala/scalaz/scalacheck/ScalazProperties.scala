...
object ScalazProperties {
  ...
  object monadPlus {
    ...
    def leftDistribution[F[_], X, Y](implicit F: MonadPlus[F], afx1: Arbitrary[F[X]], afxf: Arbitrary[X => F[Y]], ef: Equal[F[Y]]) =
      forAll(F.monadPlusLaw.leftDistribution[X, Y] _)
    ...
    def laws[F[_]](implicit F: MonadPlus[F], afx: Arbitrary[F[Int]], afy: Arbitrary[F[Int => Int]], ef: Equal[F[Int]]) = new Properties("monad plus") {
      ...
      property("left distribution") = leftDistribution[F, Int, Int]
    }
