...
trait FoldableOps[F[_],A] extends Ops[F[A]] {
  ...
  final def toNel: Option[NonEmptyList[A]] = F.toNel(self)
  final def toZipper: Option[Zipper[A]] = F.toZipper(self)
  final def zipperEnd: Option[Zipper[A]] = F.zipperEnd(self)
  final def findM[M[_] : Monad](p: A => M[Boolean]): M[Option[A]] = F.findM(self)(p)
