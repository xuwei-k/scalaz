...
final class Foldable1Ops[F[_],A] private[scalaz] (override val self: F[A])(implicit private[this] val F: Foldable1[F]) extends Ops[F[A]] {
  ////
...
trait ToFoldable1Ops0 {
  implicit final def ToFoldable1OpsUnapply[FA](v: FA)(implicit F0: Unapply[Foldable1, FA]) =
    new Foldable1Ops[F0.M,F0.A](F0(v))(F0.TC)

}

trait ToFoldable1Ops extends ToFoldable1Ops0 with ToFoldableOps {
  implicit final def ToFoldable1Ops[F[_],A](v: F[A])(implicit F0: Foldable1[F]) =
...
trait Foldable1Syntax[F[_]] extends FoldableSyntax[F] {
  implicit final def ToFoldable1Ops[A](v: F[A]): Foldable1Ops[F, A] = new Foldable1Ops[F,A](v)(Foldable1Syntax.this.F)
