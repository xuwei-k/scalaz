...
trait ToFunctorOps2 extends ToInvariantFunctorOps2 {
  implicit def ToFunctorOps2[F[_, _], A, B](v: F[A, B])(implicit F0: Functor[({type l[x] = F[A, x]})#l]): FunctorOps[({type l[x] = F[A, x]})#l, B] =
    new FunctorOps[({type l[x] = F[A, x]})#l, B](v)
}
