...
final class FunctorOps[F[_],A] private[scalaz] (override val self: F[A])(implicit private[this] val F: Functor[F]) extends Ops[F[A]] {
  ////
  import Leibniz.===
...
trait ToFunctorOps0 {
  implicit final def ToFunctorOpsUnapply[FA](v: FA)(implicit F0: Unapply[Functor, FA]) =
    new FunctorOps[F0.M,F0.A](F0(v))(F0.TC)

}

trait ToFunctorOps extends ToFunctorOps0 with ToInvariantFunctorOps {
  implicit final def ToFunctorOps[F[_],A](v: F[A])(implicit F0: Functor[F]) =
...
trait FunctorSyntax[F[_]] extends InvariantFunctorSyntax[F] {
  implicit final def ToFunctorOps[A](v: F[A]): FunctorOps[F, A] = new FunctorOps[F,A](v)(FunctorSyntax.this.F)
