...
final class ZipOps[F[_],A] private[scalaz] (override val self: F[A])(implicit private[this] val F: Zip[F]) extends Ops[F[A]] {
  ////
...
trait ToZipOps0 {
  implicit final def ToZipOpsUnapply[FA](v: FA)(implicit F0: Unapply[Zip, FA]) =
    new ZipOps[F0.M,F0.A](F0(v))(F0.TC)

}

trait ToZipOps extends ToZipOps0 {
  implicit final def ToZipOps[F[_],A](v: F[A])(implicit F0: Zip[F]) =
    new ZipOps[F,A](v)
...
trait ZipSyntax[F[_]]  {
  implicit final def ToZipOps[A](v: F[A]): ZipOps[F, A] = new ZipOps[F,A](v)(ZipSyntax.this.F)
