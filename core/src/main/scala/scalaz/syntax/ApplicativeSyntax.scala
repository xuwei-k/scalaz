...
final class ApplicativeOps[F[_],A] private[scalaz] (override val self: F[A])(implicit private[this] val F: Applicative[F]) extends Ops[F[A]] {
  ////
...
trait ToApplicativeOps0 {
  implicit final def ToApplicativeOpsUnapply[FA](v: FA)(implicit F0: Unapply[Applicative, FA]) =
    new ApplicativeOps[F0.M,F0.A](F0(v))(F0.TC)

}

trait ToApplicativeOps extends ToApplicativeOps0 with ToApplyOps {
  implicit final def ToApplicativeOps[F[_],A](v: F[A])(implicit F0: Applicative[F]) =
...
trait ApplicativeSyntax[F[_]] extends ApplySyntax[F] {
  implicit final def ToApplicativeOps[A](v: F[A]): ApplicativeOps[F, A] = new ApplicativeOps[F,A](v)(ApplicativeSyntax.this.F)
