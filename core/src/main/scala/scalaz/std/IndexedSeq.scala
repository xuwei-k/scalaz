...
trait IndexedSeqSubInstances extends IndexedSeqInstances0 with IndexedSeqSub {self =>
  val ixSqInstance = new Traverse[IxSq] with MonadPlus[IxSq] with Each[IxSq] with Index[IxSq] with Length[IxSq] with Zip[IxSq] with IsEmpty[IxSq] {
    ...
    override def unzip[A, B](a: IxSq[(A, B)]) = a.unzip
