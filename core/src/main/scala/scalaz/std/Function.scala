...
trait FunctionInstances extends FunctionInstances0 {
  ...
  implicit def function1Covariant[T]: Monad[({type l[a] = (T => a)})#l] with Zip[({type l[a] = (T => a)})#l] with Distributive[({type l[a] = (T => a)})#l] = new Monad[({type l[a] = (T => a)})#l] with Zip[({type l[a] = (T => a)})#l] with Distributive[({type l[a] = (T => a)})#l] {
    def point[A](a: => A) = _ => a
    ...
    override def unzip[A, B](a: T => (A, B)) =
      (a(_)._1, a(_)._2)
