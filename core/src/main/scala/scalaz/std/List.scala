...
trait ListInstances extends ListInstances0 {
  implicit val listInstance = new Traverse[List] with MonadPlus[List] with Each[List] with Index[List] with Length[List] with Zip[List] with IsEmpty[List] with Cobind[List] {
    ...
    override def unzip[A, B](a: List[(A, B)]) = a.unzip
