...
trait OrderingInstances {
  def orderingMonoid[A]: Monoid[SOrdering[A]] = new AbstractMonoid[SOrdering[A]] {
...
trait OrderingFunctions {
  final def ToScalazOrderFromOrdering[A](oa: SOrdering[A]): scalaz.Order[A] = new AbstractOrder[A] {
