...
object Gen extends GenInstances0 {
  private[scalaz] val defaultSize = Platform.genSize

  private[this] val iListFromList = IList.fromList[Any] _
  private[scalaz] def IListFromList[A]: List[A] => IList[A] =
    iListFromList.asInstanceOf[List[A] => IList[A]]

  private[scalaz] val Int2Byte = (_: Int).toByte
  private[scalaz] val Int2Short = (_: Int).toShort
  ...
  private[scalaz] def treeGenSized[A: NotNothing](size: Int)(implicit A: Gen[A]): Gen[Tree[A]] =
    size match {
      case n if n <= 1 =>
        A.map(a => Tree.Leaf(a))
      case 2 =>
  ...
  private[scalaz] def treeLocGenSized[A](size: Int)(implicit A: Gen[A]): Gen[TreeLoc[A]] = {
