...
sealed abstract class SelectTInstances6 {
...
sealed abstract class SelectTInstances0 extends SelectTInstances1 {
...
sealed abstract class SelectTInstances extends SelectTInstances0 {
  implicit def selectTMonadTrans[R]: MonadTrans[Lambda[(x[_], y) => SelectT[R, x, y]]] =
    new MonadTrans[Lambda[(x[_], y) => SelectT[R, x, y]]] {
      ...
      override def apply[G[_] : Monad]: Monad[SelectT[R, G, ?]] =
...
private trait SelectTMonad[R, M[_]] extends Monad[SelectT[R, M, ?]] with SelectTBind[R, M] {
  protected implicit def F: Monad[M]
...
private trait SelectTPlus[R, M[_]] extends Plus[SelectT[R, M, ?]] {
  protected implicit def F: Plus[M]
...
private trait SelectTPlusEmpty[R, M[_]] extends PlusEmpty[SelectT[R, M, ?]] with SelectTPlus[R, M] {
  protected implicit def F: PlusEmpty[M]
...
private trait SelectTMonadPlus[R, M[_]] extends MonadPlus[SelectT[R, M, ?]] with SelectTPlusEmpty[R, M] with SelectTMonad[R, M] {
  protected implicit def F: MonadPlus[M]
}
