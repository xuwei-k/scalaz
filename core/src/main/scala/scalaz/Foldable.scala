...
trait Foldable[F[_]]  { self =>
  ...
  def maximum[A: Order](fa: F[A]): Option[A] =
    foldLeft(fa, none[A]) {
      case (None, y) => some(y)
      case (xx @ Some(x), y) => if (Order[A].order(x, y) == GT) xx else some(y)
    }
  ...
  def maximumOf[A, B: Order](fa: F[A])(f: A => B): Option[B] =
    foldLeft(fa, none[B]) {
      ...
      case (b0 @ Some(b), aa) => val bb = f(aa); if (Order[B].order(b, bb) == GT) b0 else some(bb)
    }
  ...
  def maximumBy[A, B: Order](fa: F[A])(f: A => B): Option[A] =
    foldLeft(fa, none[(A, B)]) {
      ...
      case (x @ Some((_, b)), aa) => val bb = f(aa); if (Order[B].order(b, bb) == GT) x else some(aa -> bb)
    } map (_._1)
  ...
  def minimum[A: Order](fa: F[A]): Option[A] =
    foldLeft(fa, none[A]) {
      case (None, y) => some(y)
      case (xx @ Some(x), y) => if (Order[A].order(x, y) == LT) xx else some(y)
    }
  ...
  def minimumOf[A, B: Order](fa: F[A])(f: A => B): Option[B] =
    foldLeft(fa, none[B]) {
      case (None, a) => some(f(a))
      case (b0 @ Some(b), aa) => val bb = f(aa); if (Order[B].order(b, bb) == LT) b0 else some(bb)
    }
  ...
  def minimumBy[A, B: Order](fa: F[A])(f: A => B): Option[A] =
    foldLeft(fa, none[(A, B)]) {
      case (None, a) => some(a -> f(a))
      case (x @ Some((_, b)), aa) => val bb = f(aa); if (Order[B].order(b, bb) == LT) x else some(aa -> bb)
    } map (_._1)
