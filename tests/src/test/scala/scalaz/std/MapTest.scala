...
abstract class XMapTest[Map[K, V] <: SMap[K, V] with MapLike[K, V, Map[K, V]], BKC[_]]
  ...
  "getOrAdd" ! forAll { (m0: Map[Int, Long], k: Int, vOld: Long, vNew: Long) =>
    ...
    // not already in map
    getOrAdd[Id, Int, Long](mWithout, k)(vNew) must_=== (mWithNew, vNew)

    // already in map
    getOrAdd[Id, Int, Long](mWithOld, k)(vNew) must_=== (mWithOld, vOld)

    // lazy
    var evaluated = false
    getOrAdd[Id, Int, Long](mWithOld, k)({evaluated = true; vNew}) must_=== (mWithOld, vOld)
